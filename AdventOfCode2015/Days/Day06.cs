...
namespace AdventOfCode2015.Days
{
    public class Day06
    {
        ...
        private string Stage1()
        {
            ...
            Dictionary<(int x, int y), bool> lights = new Dictionary<(int x, int y), bool>();
            for (int x = 0; x < 1000; x++)
            {
                for (int y = 0; y < 1000; y++)
                {
                    lights.Add((x, y), false);
                }
            }

            foreach (var command in commands)
            {
                var bla = command.Split(" ");

                if (bla[0] == "turn")
                {
                    (int x, int y) from = GetXY(bla[2]);
                    (int x, int y) to = GetXY(bla[4]);
                ...
                if (bla[0] == "toggle")
                {
                    (int x, int y) from = GetXY(bla[1]);
                    (int x, int y) to = GetXY(bla[3]);
            ...
            return lights.Where(d => d.Value).Count().ToString();
        }

        private (int x, int y) GetXY(string input)
        {
        ...
        private string Stage2()
        {
            ...
            Dictionary<(int x, int y), int> lights = new Dictionary<(int x, int y), int>();
            for (int x = 0; x < 1000; x++)
            {
                for (int y = 0; y < 1000; y++)
                {
            ...
            return lights.Select(d => d.Value).Sum().ToString();
        }
    }
}
